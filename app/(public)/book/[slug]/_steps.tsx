...
/**
 * Every step of the guest booking flow, as presentational components.
 *
 * Moved out of `page.tsx` when the page gained theme shells: the steps are the
 * part that does NOT change between themes, so they are shared and each shell
 * only decides where they sit and what frames them. A step therefore takes its
 * data and its callbacks as props and owns no navigation of its own — the flow
 * hook (`_use-booking-flow`) decides what comes next for every theme at once.
 */

import { useState, useMemo } from "react";
import { Calendar, Check, ClipboardList, Clock, Minus, Plus, Sparkles, Users } from "lucide-react";
      ...
      {steps.map((s, i) => (
        <li
          ...
          className={cn(
            ...
            // Only items with a trailing connector need to grow; the last one
            // sizing to its content keeps the rail from ending in empty space.
        ...
        >
          <span
            className={cn(
              ...
              i < idx && "bg-[color:var(--bk-success-bg,#d1fae5)] text-[color:var(--bk-success-text,#047857)]",
          ...
          {/* Only the step you are on is named on screen, and it is named at
              title size: with the other labels gone it is the one thing saying
              where you are. Sized against the 28px markers rather than as a
              real heading — 14px on phones, 16px above, so it leads the rail
              without dwarfing it or pushing the connectors off the row. The
              rest keep their label for screen readers, which would otherwise
              get a list of bare numbers with nothing to say what any of them
              is. */}
          <span
            className={cn(
              i === idx
...
function JoinWaitlistPanel({
  ...
  return (
    <div className="mt-6 space-y-4 rounded-xl border border-[color:var(--bk-notice-border,#fde68a)] bg-[color:var(--bk-notice-bg,#fffbeb)] p-4">
      <div className="flex items-start gap-2">
        <ClipboardList className="mt-0.5 h-4 w-4 text-[color:var(--bk-notice-text,#92400e)]" />
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[color:var(--bk-notice-text,#78350f)]">Join the waitlist</p>
          <p className="text-xs text-[color:var(--bk-notice-text,#78350f)]">
      ...
      {doneMsg ? (
        <p
          className={cn(
            "text-xs",
            doneMsg.includes("on the list") ? "text-[color:var(--bk-success-text,#065f46)]" : "text-destructive",
          )}
        >
          {doneMsg}
        </p>
...
export function StepDone({
  ...
  return (
    <Card className="p-8 text-center">
      <div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[color:var(--bk-success-bg,#d1fae5)] text-[color:var(--bk-success-text,#047857)]">
